...
The first interesting finding is that even all white wines are labeled with alcohol % in volume, the residual sugars, or sweetness also matters according to the data.  This can be seen in the below graph:
...
The next interesting finding is that the quality of the white wines is mainly judged by alcohol.  A trend can be found in the following box plot:
...
Finally, alcohol, density and quality are found to be correlated in the above analysis and we can generalize them in one graph:

```{r, fig.width=12, fig.height=8, Plot_Three}
ggplot(aes(alcohol, density, color=quality.f), data=ww) +
